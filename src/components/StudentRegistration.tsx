  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if(formData.rollNumber){
        const q = query(collection(db,'students'), where('rollNumber','==', formData.rollNumber), limit(1));
      ...
      const studentData: Omit<Student, 'id'> = {
        ...
        classLevel: formData.class, // legacy mirror
      ...
      // Save to Firestore first (we need ID for password)
      await setDoc(studentRef, studentData);

      // If a student email was provided, create an auth user for the student.
      // Password = studentId (document id). We warn teacher about sharing it securely.
      let studentAuthError: string | null = null;
      if (formData.studentEmail) {
        try {
          await createUserWithEmailAndPassword(auth, formData.studentEmail.trim(), studentRef.id);
          // Create/merge a users doc for student role
          await setDoc(doc(collection(db, 'users'), studentRef.id), {
            role: 'student',
          ...
          console.error('Student auth account creation failed', e);
          studentAuthError = e.message || 'Failed to create student login';
        }
      }
      ...
      toast.success(
        (isAdmin ? 'Student registered (QR generated). ' : 'Student registered. ') +
        (formData.studentEmail ? 'Login created.' : 'No student email provided.')
      );
      if (formData.studentEmail) {
        toast.custom(<div className="bg-white text-sm text-midnight_text px-4 py-3 rounded shadow max-w-sm">Student initial password is <strong>{studentRef.id}</strong>. Advise student to change it after first login.</div>, { duration: 8000 });
      }
      if (studentAuthError) {
        toast.error('Student login issue: ' + studentAuthError);
      }
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary focus:border-primary"
            />
            <p className="text-xs text-grey mt-1">If provided, student can log in with this email and initial password = generated ID.</p>
          </div>
        </div>
