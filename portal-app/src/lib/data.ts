...
export async function searchStudents(term:string){ const col = collection(db,'students'); const all = await getDocs(col); const lower= term.toLowerCase(); return all.docs.map(d=>d.data() as StudentRecord).filter(s=> s.firstName.toLowerCase().includes(lower) || s.lastName.toLowerCase().includes(lower) || s.id.toLowerCase().includes(lower)); }
export async function fetchStudentsPage(pageSize=10, cursor?:any){ const colRef = collection(db,'students'); let q = query(colRef, orderBy('createdAt','desc'), limit(pageSize)); if(cursor) q = query(colRef, orderBy('createdAt','desc'), startAfter(cursor), limit(pageSize)); const snap = await getDocs(q); const students = snap.docs.map(d=>d.data() as StudentRecord); const nextCursor = snap.docs.length===pageSize? snap.docs[snap.docs.length-1] : null; return { students, cursor: nextCursor }; }
